...
# --- 2. EMAIL UTILITY (THE ULTIMATE SCRUB) ---
def send_email(recipient, buyer_name, content):
    try:
        ...
        # --- THE FIX: MANUAL CHARACTER SCRUBBING ---
        # This physically hunts down the characters causing the ASCII error
        clean_body = body.replace('\xa0', ' ').replace('\u202f', ' ')
        
        # We attach using UTF-8 and 'strict' handling to ensure it doesn't revert to ASCII
        part = MIMEText(clean_body, 'plain', 'utf-8')
        msg.attach(part)
...
# --- 3. DATA LOAD (LOCAL/CLOUD HYBRID) ---
def load_pricing():
                            ...
                            "content": [
                                {
                                    ...
                                    3. MATH RULE: Weekly Revenue Loss = (list_price * 7 * weekly_velocity * 1). 
                                       - Show full math (e.g., $4.49 * 7 * 4 * 1).
                                    
                                    4. STRATEGIC PITCH & FINANCIAL ANALYSIS: Write two bolded paragraphs.
                                       - MANDATORY: BOLD the total aggregate weekly revenue loss across all 6 OOS items. (No headers).
